...
def generate_batch(image):
    ...
    for i in range(0,image.qsize()):
        ...
        im_info = np.array([[im_array.shape[2], im_array.shape[3], im_scalet]], dtype=np.float32)
        t1.append(im_array.shape)
        t2.append(im_info.shape)
        data.append([mx.nd.array(im_array), mx.nd.array(im_info)])
        data_shapes.append([('data', im_array.shape), ('im_info', im_info.shape)])
...
def main():
    ...
    video = Queue.Queue(maxsize = batch_size)
    cnt=0
    for image in imgs:
        print(os.path.join(args.in_dir, image))
        video.put(cv2.imread(os.path.join(args.in_dir, image)))
        cnt+=1
        if(cnt==batch_size):
            demo_net(predictor, video, args)
            cnt=0
            video.clear()
        num += 1
