...
def main(image, prefix, epoch, gpu, vis, network, in_dir, test, out_dir, label_dir, with_label):
    ...
    while (cap.isOpened()):
        ...
        # print(x1,y1,w1,h1)
            ...
            # cv2.imwrite("tmp.jpg", frame)
                x1, y1, w1, h1 = main(video[(k-24)%24], prefix='model/e2e', epoch=20, gpu=0, vis=False, network='vggm',
                ...
                for i in range(0, len(x1)):
                    ...
                    w = w1[i]
                    h = h1[i]
                    cv2.rectangle(video[(k-24)%24], (ix, iy), (ix + w, iy + h), (0, 255, 255), 2)
                    tracker = kcftracker.KCFTracker(True, True, True)  # hog, fixed_window, multiscale
                    tracker.init([ix, iy, w, h],video[(k-24)%24])
                # print (tracker)
                    trackers[i] = tracker
                # print (trackers[i])
            # print (len(trackers))
            ...
            # print(ix, iy, w, h)
                ...
                for i in range(0, len(result)):
                    res = result[i].get()
                    cv2.rectangle(video[t], (res[0], res[1]),
                              (res[0] + res[2], res[1] + res[3]), (0, 255, 255), 1)
                t1 = time()
