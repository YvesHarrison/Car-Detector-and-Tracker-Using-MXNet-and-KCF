...
def demo_net(lock,predictor, image_name, image, with_label, vis, out_dir, label_dir):
    """
    ...
    data_batch, data_names, im_scale = generate_batch(im,lock)
    # for i in range(10):
...
def main(lock,predictor,ctx,image, prefix, epoch, gpu, vis, network, in_dir, test, out_dir, label_dir, with_label):
    ...
    if in_dir=='':
        ...
        a=len(image)
        for i in range (0,a):
            x1, y1, w1, h1 = demo_net(lock,predictor,image[i], image[i], with_label, vis, out_dir,
                                     label_dir)
            #print(x1, y1, w1, h1)
            x.append(x1)
            y.append(y1)
...
def video(lock,predictor,name,ctx):
    inteval = 1
    duration = 0.01
    global selectingObject, initTracking, onTracking, ix, iy, cx, cy, w, h
    show_delay=24
    ...
    while (cap.isOpened()or ct!=cn):
        ...
        if store:
            ...
            if (selectingObject):
                ...
                x1, y1, w1, h1 = main(lock,predictor,ctx,capture, prefix='model/e2e', epoch=20, gpu=0, vis=False, network='vggm',
...
def video_capture(name,network_inteval):
    cap = cv2.VideoCapture(name)
    k=0
    video = []
    capture=[]
...
def play_video(video,res_x,res_y,res_w,res_h):
    inteval = 1
    duration = 0.01
    global selectingObject, initTracking, onTracking, ix, iy, cx, cy, w, h
    network_inteval=8
    ...
    while (k!=len(video)):
          ...
          if (initTracking):
              print(k)
              x=res_x[k/network_inteval]
              y=res_y[k/network_inteval]
              w=res_w[k/network_inteval]
              h=res_h[k/network_inteval]
              initTracking = False
              onTracking = True
              length=len(x)
              show=video[k]
...
if __name__ == '__main__':
    ...
    for i in range(0, len(sys.argv)-1):
        result.append(pool.apply_async(video_capture, (name[i], 8)))
    video=[]
    capture=[]
    for i in range(0, len(sys.argv)-1):
        res = result[i].get()
        video.append(res[0])
        capture.append(res[1])
    ctx = mx.gpu(gpu)
    network='vggm'
    prefix='model/e2e'
    epoch=20
    ...
    lock=0
    for i in range(0, len(sys.argv)-1):
        x1,y1,w1,h1=main(lock,predictor,ctx,capture[i], prefix='model/e2e', epoch=20, gpu=0, vis=False, network='vggm',
                                  in_dir='', test='test.txt', out_dir='output/',
                                  label_dir='data/hcar/Annotations/', with_label=1)
        res_x.append(x1)
        res_y.append(y1)
    ...
    record=[]
    for i in range(0,len(sys.argv)-1):
        process = multiprocessing.Process(target=play_video, args=(video[i],res_x[i],res_y[i],res_w[i],res_h[i]))
        process.start()
    ...
    #pool=multiprocessing.Pool(len(name))
    for i in range(0, len(sys.argv)-1):
        #pool.apply_async(video,(lock,predictor,str(name[i]),ctx))
        process = multiprocessing.Process(target=video, args=(lock,predictor,str(name[i]),ctx,))
        process.start()
        record.append(process)
                # print(boundingbox)
    for process in record:
