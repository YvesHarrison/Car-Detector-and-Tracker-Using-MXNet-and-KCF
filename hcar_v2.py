...
def get_pre_box(im, im_scale):
    h = im.shape[0] * im_scale
    w = im.shape[1] * im_scale
    #print('w,h', w, h)
    pre_box = np.array([ w*0.6, 0, w-1, h*0.4, 1 ])

    return pre_box

def is_valid(box, im, im_scale):
    #h = im.shape[0] * im_scale
    #w = im.shape[1] * im_scale
    #print('w,h', w, h)
    #pre_box = [ w*0.67, 0, w-1, h*0.33 ]
    #print('box', box)
    #print('pre_box', pre_box)

    pre_box = get_pre_box(im, im_scale)

    print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxx is_valid:', intersect(box, pre_box) / area(box) < 0.5)
    return intersect(box, pre_box) / area(box) < 0.5
...
def demo_net(predictor, image_name, args):
    ...
    bcs = [ (list(box), cls) for (cls, boxes) in rst.items() for box in boxes if is_valid(box, im, im_scale)]
    ...
    rst = tmp_rst
    boxes_this_image =[ copy.deepcopy(rst[CLASSES[idx]]) if CLASSES[idx] in rst else []  for idx in range(len(boxes_this_image)) ]
