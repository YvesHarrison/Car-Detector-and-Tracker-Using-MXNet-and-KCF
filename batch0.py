        ...
        if store:
            ...
            if (selectingObject):
                ...
                for i in range(3, 0, -1):
                    x1, y1, w1, h1 = main(predictor,video[(k - 8 * i) % 24], prefix='model/e2e', epoch=20, gpu=0, vis=False,
                                          network='vggm',
                ...
                if (t == 0 or t == 8 or t == 16):
                    ...
                    for i in range(0, n[j]):
                        ...
                        iw = w[j][i]
                        ih = h[j][i]
                        cv2.rectangle(video[(k - 24) % 24], (ix, iy), (ix + iw, iy + ih), (0, 255, 255), 2)
                        tracker = kcftracker.KCFTracker(True, True, True)  # hog, fixed_window, multiscale
                        tracker.init([ix, iy, iw, ih], video[(k - 24) % 24])
                        # print (tracker)
                        trackers[i] = tracker
                        # print (trackers[i])
                        # print (len(trackers))
                ...
                # duration = t1-t0
                cv2.putText(video[t], 'FPS: ' + str(1 / duration)[:4].strip('.'), (8, 20), cv2.FONT_HERSHEY_SIMPLEX,
                            0.6,
                            (0, 0, 255), 2)
                if (timer == 8):
                    timer = 0
