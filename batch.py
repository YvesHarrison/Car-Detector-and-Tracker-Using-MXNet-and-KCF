...
def main(predictor, image, prefix, epoch, gpu, vis, network, in_dir, test, out_dir, label_dir, with_label):
    ...
    for image in imgs:
        ...
        x1,y1,w1,h1=demo_net(predictor, os.path.join(in_dir, image), image, with_label, vis, out_dir, label_dir)
        x.append(x1)
        y.append(y1)
        w.append(w1)
        h.append(h1)
    ...
    while (cap.isOpened()):
        ...
        if store:
            ...
            if(k%network_inteval==0):
                ...
                result_file = os.path.join('video/',name)
        ...
        # print(x1,y1,w1,h1)
            ...
            # cv2.imwrite("tmp.jpg", frame)
                ...
                t5=time()
                x1, y1, w1, h1 = main(predictor, image='', prefix='model/e2e', epoch=20, gpu=0, vis=False, network='vggm',
                                  in_dir='video/', test='test.txt', out_dir='output/',
            ...
            # print(ix, iy, w, h)
                ...
                if (t%network_inteval==0):
                    ...
                    for i in range(0, n[j]):
                        ...
                        iw = w[j][i]
                        ih = h[j][i]
                        cv2.rectangle(video[t], (ix, iy), (ix + iw, iy + ih), (0, 255, 255), 2)
                        tracker = kcftracker.KCFTracker(True, True, True)  # hog, fixed_window, multiscale
                        tracker.init([ix, iy, iw, ih], video[t])
                        trackers[i] = tracker
                    if(length!=0):
